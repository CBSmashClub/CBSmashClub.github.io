(function() {
  ...
  GalleryLightbox = function(element, options) {
    var content, footer, header;
    this.options = $.extend({
      title: null,
      footer: null,
      remote: null
    }, $.fn.galleryLightbox.defaults, options || {});
    this.$element = $(element);
    content = '';
    this.modal_id = this.options.modal_id ? this.options.modal_id : 'galleryLightbox-' + Math.floor((Math.random() * 1000) + 1);
    header = '<div class="modal-header"' + (this.options.title || this.options.always_show_close ? '' : ' style="display:none"') + '><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">' + (this.options.title || "&nbsp;") + '</h4></div>';
    footer = '<div class="modal-footer"' + (this.options.footer ? '' : ' style="display:none"') + '>' + this.options.footer + '</div>';
    $(document.body).append('<div id="' + this.modal_id + '" class="gallery-lightbox modal fade" tabindex="-1"><div class="modal-dialog"><div class="modal-content">' + header + '<div class="modal-body"><div class="gallery-lightbox-container"><div></div></div></div>' + footer + '</div></div></div>');
    ...
    this.lightbox_container = this.modal_body.find('.gallery-lightbox-container').first();
    ...
    this.modal.on('show.bs.modal', this.options.onShow.bind(this)).on('shown.bs.modal', (function(_this) {
      ...
      return function() {
        if (_this.gallery) {
          $(document).off('keydown.galleryLightbox');
        }
        _this.modal.remove();
  ...
  GalleryLightbox.prototype = {
    modal_shown: function() {
      ...
      if (!this.options.remote) {
        ...
        if (this.gallery) {
          ...
          $(document).on('keydown.galleryLightbox', this.navigate.bind(this));
          if (this.options.directional_arrows && this.gallery_items.length > 1) {
            this.lightbox_container.append('<div class="gallery-lightbox-nav-overlay"><a href="#" class="' + this.strip_stops(this.options.left_arrow_class) + '"></a><a href="#" class="' + this.strip_stops(this.options.right_arrow_class) + '"></a></div>');
            this.modal_arrows = this.lightbox_container.find('div.gallery-lightbox-nav-overlay').first();
    ...
    updateTitleAndFooter: function() { //help??????????????????????????????????????????
    ...
    showYoutubeVideo: function(id) {
      ...
      return this.showVideoIframe('https://www.youtube.com/embed/' + id + '?badge=0&autoplay=1&html5=1' + rel, width, height);
    },
  ...
  $.fn.galleryLightbox = function(options) {
    return this.each(function() {
      ...
      new GalleryLightbox(this, options);
      return this;
    });
  };

  $.fn.galleryLightbox.defaults = {
